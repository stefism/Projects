...
namespace _05m_Shopping_Spree
{
    ...
    class Program
    {
        ...
        static void AddProductToList(List<PeoplesAndProducts> byedItemsForPrint, string personName, string productName)
        {
            foreach (var item in byedItemsForPrint)
            {
                if (item.PeopleName == personName)
                {
                    PeoplesAndProducts product = new PeoplesAndProducts();
                    product.BuyedProducts.Add(productName); 
                }
            }
        }
            ...
            foreach (var item in persons)
            {
                if (item.PersonName == personName)
                {
                    ...
                    if (item.PersonMoney >= productPrice)
                    {
                        ...
                        PeoplesAndProducts currentProduct = new PeoplesAndProducts();
                        currentProduct.PeopleName = item.PersonName;
                        currentProduct.BuyedProducts.Add(productName);
                        ByedItemsForPrint.Add(currentProduct);
                    }
                    else
                    {
