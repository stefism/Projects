...
namespace P06_Sneaking
{
    public class Engine
    {
        ...
        public void Run()
        {
            int rowsInRoom = int.Parse(Console.ReadLine());

            room = new char[rowsInRoom][];

            InititializeMatrix(rowsInRoom);

            var moves = Console.ReadLine().ToCharArray();

            int[] samPosition = new int[2];

            DefineSamPosition(samPosition);
            ...
            for (int i = 0; i < moves.Length; i++)
            {
                ...
                MoveSamInPosition(moves, samPosition, i);
        ...
        private static void MoveSamInPosition(char[] moves, int[] samPosition, int i)
        {
            switch (moves[i])
            {
                case 'U':
        ...
        private static void DefineSamPosition(int[] samPosition)
        {
            for (int row = 0; row < room.Length; row++)
            {
                for (int col = 0; col < room[row].Length; col++)
        ...
        private static void InititializeMatrix(int rowsInRoom)
        {
